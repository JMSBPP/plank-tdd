...
5. Ask which **one** behavior to lock. Fill only the holes that behavior needs.
6. **Every define writes behavioral semantics as a `.btt` file, then Bulloak generates the Foundry suite.** Do not hand-roll `*.t.sol` for a define slice. See [REFERENCE.md — BTT / Bulloak](../REFERENCE.md#btt--bulloak-every-define).
...
7. Side effects: implement `io` wrap + `run`/`run_io` for that behavior; `xfer`/`view` only for selectors on the Eff row.
8. Add the harness `.plk` if new. Add every new `.plk` to the domain `compile.toml`.
9. Any args after `/plank-define` are the behavior to define — confirm algebra still matches, then implement that slice only.
